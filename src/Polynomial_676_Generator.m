% === Step 1: Define fixed Cartesian path points & Orientation ===
% https://www.elephantrobotics.com/en/mycobot-280-m5-new-specificatons-en/

% Target Orientation: Facing Down (Gripper points -Z)
% Roll=pi (180), Pitch=0, Yaw=0
target_rpy = [pi, 0, 0]; 

p_start   = [200, 100, 100];  % Start point
p_lift    = [200, 100, 180];  % Lift-up point
p_descent = [150, 120, 140];  % Descent point
p_end     = [100, 150, 100];  % Final destination
...
% Now passing 6 arguments: X, Y, Z, Roll, Pitch, Yaw
fprintf('Solving Inverse Kinematics with Orientation Control...\n');

[theta1, ~] = ilink6dofelephant(p_start(1),   p_start(2),   p_start(3),   target_rpy(1), target_rpy(2), target_rpy(3));
[theta2, ~] = ilink6dofelephant(p_lift(1),    p_lift(2),    p_lift(3),    target_rpy(1), target_rpy(2), target_rpy(3));
[theta3, ~] = ilink6dofelephant(p_descent(1), p_descent(2), p_descent(3), target_rpy(1), target_rpy(2), target_rpy(3));
[theta4, ~] = ilink6dofelephant(p_end(1),     p_end(2),     p_end(3),     target_rpy(1), target_rpy(2), target_rpy(3));
...
function [G, E] = ilink6dofelephant(xd, yd, zd, r_t, p_t, y_t)
    % ILINK6DOFELEPHANT Solves Inverse Kinematics for MyCobot 280 using PSO.
    % Inputs: xd, yd, zd, roll, pitch, yaw
    % Outputs: G (Best Joint Angles), E_history (Convergence Plot Data)
    
    N=6;                       % DOF
    target_pos = [xd, yd, zd]; % Target Position
    target_rot = rpy2rotm(r_t, p_t, y_t); % Target Rotation Matrix
    
    ww=0.8;                    % Inertia weight
    c1=1.2;                    % Cognitive Weight
    c2=1.2;                    % Social Weight
    r=rand(1,2);               % 
    H=200;                     % Number of Particles
    N_iter=1000; 
    vmin=-1; 
    vmax=1;
    ...
    while iter < N_iter
        ...
        for i=1:H
            ...
            EB(i)=objfxyz(B(i,:), target_pos, target_rot);
        ...
        for i=1:H
            ...
            EP=objfxyz(P(i,:), target_pos, target_rot);
...
function E = objfxyz(BB, HDj, TargetR)
    % Extract angles
    ...
    % 1. Orientation Error (Frobenius Norm)
    err_rot = norm(TargetR - current_R, 'fro');
    
    % 2. Hard Constraint: Kill any solution that is grossly tilted
    % 0.5 is roughly 30 degrees of error. We kill anything worse than that.
    if err_rot > 0.5
        E = 1e6; % Return a massive number (Soft Infinity)
        return;
    end

    % 3. Position Error (Squared Distance)
    err_pos = sum((current_pos - HDj).^2);
    
    % 4. Joint limits penalty (Soft constraint for limits)
    % (Your main loop handles hard limits, but this helps guide it)
    penalty = 0; 
    
    % --- WEIGHTING ---
    % We prioritize Orientation highest, then Position.
    % We largely ignore joint effort (set to 0.001) to allow "awkward" poses if needed.
    
    w_pos = 1.0;
    w_rot = 500.0;   % <--- MASSIVELY INCREASED WEIGHT
    w_effort = 0.001; 
    
    E = (w_pos * err_pos) + (w_rot * err_rot) + (w_effort * sum(abs(BB)));
end